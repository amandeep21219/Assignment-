...
import com.example.PdfReader.PdfHelper.PdfTextExtractor;
import com.example.PdfReader.PdfHelper.SearchService;
import com.example.PdfReader.SemanticSearchHelper.ChunkingService;
import com.example.PdfReader.SemanticSearchHelper.OpenAIEmbeddingService;
import com.example.PdfReader.SemanticSearchHelper.SemanticSearchService;
...
public class PdfServiceImpl implements PdfService {
    ...
    public PdfServiceImpl(PdfTextExtractor pdfTextExtractor,
        ...
        try {
            ...
            try {
                ...
                if (!chunks.isEmpty()) {
                    ...
                    List<List<Double>> embeddings = openAIembeddingService.generateBatchEmbeddings(chunkTexts);
     ...
     * @return
     */
    public SearchResponse searchInUploaded(UUID fileId, String query, Boolean caseSensitive, Integer contextChars, Boolean useSemantic,PageRequest pageRequest,boolean pagination) {
        ...
        int fromIndex = Math.min(offset, totalMatches);
        int toIndex = Math.min(offset + limit, totalMatches);

        List<Match> paginatedMatches = matches.subList(fromIndex, toIndex);

        return new SearchResponse(fileId, document.getFilename(), query, paginatedMatches, totalMatches);
    }
    ...
    public SearchResponse uploadAndSearch(MultipartFile file, String query, Boolean caseSensitive, Integer contextChars, Boolean useSemantic,PageRequest pageRequest,boolean Pagination) {
        ...
        try {
            ...
            return searchInUploaded(fileId, query, caseSensitive, contextChars, useSemantic, pageRequest,Pagination);
